...
console.log(`QANTAS DEV Org: clientid: ${clientId} clientSecret: ${clientSecret} environment: ${environment}`);
//QANTAS ORG
...
const authvalidation = function(req, res, next) {
    ...
    if((req.cookies && !(req.cookies.session && sessionMap[req.cookies.session])) && req.url.indexOf('oauth') == -1)
    {
        ...
        var redirectUri=`https://login.mypurecloud.com.au/oauth/authorize?response_type=code&client_id=${clientId}&redirect_uri=${originloc}/dev/oauth2/callback`;
...
app.get('/oauth2/callback', async function(req,res){
    ...
    var originloc =getFormattedUrl(req);
    console.log(`oauth callback originloc: ${originloc}'/dev/oauth2/callback`);

    const params = new URLSearchParams();
    params.append('grant_type', 'authorization_code');
    params.append('code', authCode);
    //params.append('redirect_uri', `http://localhost:${PORT}/oauth2/callback`);
    params.append('redirect_uri', `${originloc}/dev/oauth2/callback`);
    ...
    .then(response => {
        ...
        console.log(`oauth callback @@ origURl:  https://i0fz0kuui3.execute-api.ap-southeast-2.amazonaws.com/dev${origURL}`);
        res.redirect(`https://i0fz0kuui3.execute-api.ap-southeast-2.amazonaws.com/dev${origURL}`);
    })
...
app.get('/', function(req, res){
    ...
    //window.close();
    res.render('https://i0fz0kuui3.execute-api.ap-southeast-2.amazonaws.com/dev/info.html');
})
                ...
                participants.forEach(function (participant) {
                    ...
                    if(purpose=='agent'){
                            ...
                            if(participant_userid==userid)
                            {
                                ...
                                console.log(`updatewrapup Genesys API found agent participantid: ${agent_participantid} participant:agent`);
                            }
                    }
